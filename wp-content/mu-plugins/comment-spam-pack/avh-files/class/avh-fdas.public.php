...
class AVH_FDAS_Public {
	...
	function actionHandleCronCleanNonce ()
	{
		...
		if ( is_array( $all ) ) {
			...
			update_site_option( $this->core->db_options_nonces, $all );
		}

		if ( $options['general']['cron_nonces_email'] ) {
			$to = get_site_option( 'admin_email' );
			$subject = sprintf( '[%s] AVH First Defense Against Spam - WPMU DEV Version - ' . __( 'Cron - Clean nonces', 'avhfdas' ), wp_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ) );
	...
	function actionHandleCronCleanIPCache ()
	{
		...
		if ( $options['general']['cron_ipcache_email'] ) {
			$to = get_site_option( 'admin_email' );
			$subject = sprintf( '[%s] AVH First Defense Against Spam - WPMU DEV Version - ' . __( 'Cron - Clean IP cache', 'avhfdas' ), wp_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ) );
			...
			if ( empty( $commentdata['comment_type'] ) ) { // If it's a trackback or pingback this has a value
				...
				if ( $nonce != $_POST['_avh_first_defense_against_spam'] ) {
					if ( 1 == $this->core->options['general']['emailsecuritycheck'] ) {
						...
						$subject = sprintf( __( '[%s] AVH First Defense Against Spam - WPMU DEV Version - Comment security check failed', 'avhfdas' ), wp_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ) );
						...
						$message .= sprintf( __( 'For more information: http://www.stopforumspam.com/search?q=%s', 'avhfdas' ), $ip ) . "\r\n\r\n";

						$blacklisturl = admin_url( 'admin.php?action=blacklist&i=' ) . $ip . '&_avhnonce=' . $this->core->avh_create_nonce( $ip );
						$message .= sprintf( __( 'Add to the local blacklist: %s', 'avhfdas' ), $blacklisturl ) . "\r\n";
					...
					if ( ! empty( $this->core->options['sfs']['sfsapikey'] ) ) {
						...
						if ( ! ($this->checkDB_Nonces( $q['_avhnonce'] )) ) {
							$option = get_site_option( $this->core->db_options_nonces );
							$option[$q['_avhnonce']] = $q['a'] . $q['e'] . $q['i'];
							update_site_option( $this->core->db_options_nonces, $option );
						}
					}
					$m = __( '<p>Cheating huh</p>', 'avhfdas' );
					$m .= __( '<p>Protected by: AVH First Defense Against Spam - WPMU DEV Version</p>', 'avhfdas' );
	 ...
	 * Only check with SFS, other checks are done during the action get_header
	 *
	 * @WordPress Action register_post
	 */
	function actionHandleRegistration ( $sanitized_user_login, $user_email, $errors ) {
		$this->actionHandleSFSAction( false );
	}
	...
	function checkStopForumSpam ( $ip ) {
		...
		if ( isset( $spaminfo['Error'] ) ) {
			...
			if ( isset( $spaminfo['Error'] ) && $options['sfs']['error'] ) {
				...
				$to = get_site_option( 'admin_email' );
				$subject = sprintf( __( '[%s] AVH First Defense Against Spam - WPMU DEV Version - Error detected', 'avhfdas' ), wp_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ) );
	...
	function handleSpammer ( $ip, $info, $whichaction='main' )
	{
		...
		if ( $sfs_email || $php_email ) {
			...
			$to = get_site_option( 'admin_email' );
			$subject = sprintf( __( '[%s] AVH First Defense Against Spam - WPMU DEV Version - Spammer detected [%s]', 'avhfdas' ), wp_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ), $ip );
			$message = '';
			...
			if ( $options['general']['use_sfs'] && $sfs_email && $whichaction == 'comment') {
				...
				$message .= "\r\n	" . sprintf( __( 'For more information: http://www.stopforumspam.com/search?q=%s', 'avhfdas' ), $ip ) . "\r\n\r\n";
			}
		...
		$blacklist_die = !empty( $info['blacklist']['time'] ) && ( 'Blacklisted' == $info['blacklist']['time'] );
		...
		if ( $sfs_die || $php_die || $blacklist_die ) {
			...
			if ( 1 == $options['general']['diewithmessage'] ) {
				if ( !empty( $info['blacklist']['time'] ) && 'Blacklisted' == $info['blacklist']['time'] ) {
	...
	function handleSpammerCache ( $info )
	{
		...
		if ( $cache_email ) {
			...
			// General part of the email
			$to = get_site_option( 'admin_email' );
			$subject = sprintf( __( '[%s] AVH First Defense Against Spam - WPMU DEV Version - Spammer detected [%s]', 'avhfdas' ), wp_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ), $info['ip'] );
			$message = '';
	...
	{
		$message .= "\r\n" . '--' . "\r\n";
		$message .= sprintf( __( 'Your blog is protected by AVH First Defense Against Spam - WPMU DEV Version v%s' ), $this->core->version ) . "\r\n";
